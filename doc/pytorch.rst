...
* get a good sampling strategy to explore adversarial samples according to the association ``cost``-``space``-``prior knowledge``,
    ...
    * fetch the sampler best suited (according to the chef's menu) to the problem you are solving by calling :py:meth:`skwdro.base.losses_torch.Loss.default_sampler`.
