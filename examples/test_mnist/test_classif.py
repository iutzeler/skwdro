...
def prepare(X, y):
    ...
    X = _X_scaled.reshape(X.shape[0], -1).clip(0., 1.)
...
def train_loop_alexnet(epochs=10):
    X_train, X_test, y_train, y_test = prepare(*load_digits(return_X_y=True))
    ...
    for epoch in range(epochs):
        ...
        for test_data, test_target in test_loader:
            ...
            for data, target in train_loader:
                ...
                print("   Train loss: {:.2f}".format(l), "acc: {:.2f}%".format(a))
            ...
            print("Test loss: {:.2f}".format(l), "acc: {:.2f}%".format(a))
    ...
    plt.plot(np.arange(len(track_test_losses)) * len(track_train_losses) // len(track_test_losses), track_test_losses)
    plt.yscale("log")

    # Accuracy plot
    plt.subplot(2, 1, 2)
    plt.loglog(track_train_accs)
    plt.loglog(np.arange(len(track_test_accs)) * len(track_train_accs) // len(track_test_accs), track_test_accs)
    plt.yscale("log")
