...
def plots(factories, trafic, pbrs, pb):
    ...
    for pbr in pbrs:
        ax.scatter(*pbr.coef_, c="g", marker='+')
        for factory, w in zip(factories, trafic):
            ax.plot([factory[0], pbr.coef_[0]], [factory[1], pbr.coef_[1]], c=cm(cn(w)))
    fig.colorbar(ScalarMappable(cmap=cm, norm=cn), ax=ax, label=r'trafic')
    ax.scatter(*pb.coef_, c="k", marker='+', s=50.)
    ax.scatter(*pb.coef_, s=100., facecolors='none', edgecolors='k')
    fig.savefig("robust_weber.png", transparent=True)
