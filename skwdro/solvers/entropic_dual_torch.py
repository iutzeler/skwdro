...
def solve_dual(wdro_problem: WDROProblem, sigma: Union[float, pt.Tensor]=pt.tensor(.1)):
    ...
    optimizer_main = torch.optim.Adam(loss.loss.parameters(), 1e-3)
    optimizer_lam = torch.optim.Adam([loss._lam], 1e-1)
    # optimizer_lam = torch.optim.LBFGS([loss._lam])

    if loss.presample:
        losses = optim_presample(30, optimizer_main, xi, xi_labels, loss)
        np.save(
                "test_pre.npy",
                losses
            )
    else:
        losses = optim_postsample(2000, optimizer_main, optimizer_lam ,  xi, xi_labels, loss)
        np.save(
                "test_post.npy",
                losses
            )
    ...
    return SolverResult(theta, intercept, lambd, losses[-1], losses=losses)
...
def optim_postsample(
    ...
    for _ in range(n_iter):
        ...
        losses.append(objective.detach().item())#pt.linalg.norm(loss._lam.grad))
